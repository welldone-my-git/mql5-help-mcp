...
- 参数：
  ...
  - `data`（import 时必填）：JSON 知识包内容
  - `import_as`（可选）：导入时覆盖库名
- 功能：
  - `export`：将本地预处理知识序列化为可分享的 JSON 包
  - `import`：从他人分享的知识包导入，**无需自己花钱跑 Haiku**
  - `stats`：查看各库的知识缓存状态 + 修复模式数量
- 示例：
  ```
  manage_knowledge("export", "EA31337")   # 导出知识包
  manage_knowledge("import", data=...) # 导入他人知识包
  manage_knowledge("stats")              # 查看统计
  ```
...
3. Claude 给出修复方案
4. record_fix(...)              → 保存到本地
5. manage_knowledge("export")  → 分享给团队
   manage_knowledge("import")  → 团队复用，节省 API 成本
```
