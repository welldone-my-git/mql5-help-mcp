...
import { BUILTIN_ROOTS, CONFIG_PATH, activePlugin, buildIndex, docIndex, externalLibFiles, getEmbeddingConfig, getDoc, loadedLibraries, queryEngine, searchDocs, } from "./document-service.js";
        ...
        try {
            ...
            switch (name) {
                ...
                case "list_fixes": {
                    ...
                    if (query) {
                        ...
                        for (const r of results) {
                            ...
                            if (r.library_key)
                                lines.push(`库: ${r.library_key}`);
                            if (r.tags)
                                lines.push(`标签: ${r.tags}`);
                            if (r.fixed_snippet)
                                lines.push("```mql5\n" + r.fixed_snippet + "\n```");
                            lines.push("---");
                        }
                ...
                case "manage_knowledge": {
                    const { action, library_key: lk, file_path: fp, import_as } = args;
                    if (action === "export") {
                        if (!lk) {
