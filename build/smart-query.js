...
export class DiagnoseEngine {
    ...
    static LOG_LINE = /^(.+?)\((\d+),(\d+)\)\s*:\s*(error|warning)\s+(\d+):\s*(.+)$/im;
    static LOG_LINE_G = /^(.+?)\((\d+),(\d+)\)\s*:\s*(error|warning)\s+(\d+):\s*(.+)$/gim;
    docIndex;
    constructor(docIndex) {
        this.docIndex = docIndex;
    }
    async diagnose(compileLog) {
        ...
        for (const m of lines) {
            ...
            if (item.migration) {
                ...
                const hint = MIGRATION_HINTS[ident] || Object.values(MIGRATION_HINTS).find(h => message.toLowerCase().includes(h.replacement.toLowerCase().split("/")[0].trim().toLowerCase()));
                if (hint) {
        ...
        items.forEach((item, idx) => {
            ...
            if (item.migration)
                out.push(`   🔁 迁移: ${item.migration}`);
            if (item.dbSolution)
                out.push(`   💡 历史方案: ${item.dbSolution}`);
            if (item.docHint)
                out.push(`   📄 参考文档: ${item.docHint}`);
