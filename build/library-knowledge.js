...
export class KnowledgeStore {
    ...
    async save(libraryKey, absPath, data) {
        const jsonFile = this.jsonPath(libraryKey, absPath);
    ...
    async loadAll(libraryKeys) {
        const all = [];
        for (const k of libraryKeys)
...
const HAIKU_USER = (relPath, content) => `
File: ${relPath}
...
export class LibraryPreprocessor {
    ...
    async processFile(libraryKey, absPath, relPath) {
        // 命中缓存直接返回
    ...
    async processLibrary(libraryKey, files, onProgress) {
...
export class ContextAssembler {
    ...
    async assemble(userCode, libraryKeys) {
        ...
        const sections = ["【已加载库的 API 知识摘要】"];
        for (const fk of allKnowledge) {
            if (fk.classes.length === 0 && fk.standalone_functions.length === 0)
                continue;
            sections.push(`\n▸ ${fk.library} / ${fk.file}`);
