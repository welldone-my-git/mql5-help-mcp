...
// ========== 结构概览 ==========
function buildOverview(code, lines) {
    ...
    let tm;
    while ((tm = tradeRe.exec(code)) !== null)
        tradeCalls.push(tm[1]);
    return {
...
const RULES = [
    // ── HIGH ──────────────────────────────────────────────────────────
    {
        ...
        check(code, lines, ov) {
            if (!ov.usesIndicators || ov.indicatorHandles.length === 0)
                return null;
            if (/\bIndicatorRelease\s*\(/.test(code))
                return null;
    ...
    // ── MEDIUM ────────────────────────────────────────────────────────
    {
        ...
        check(code, _lines, ov) {
            if (!ov.usesCTrade)
                return null;
            if (/\bSetExpertMagicNumber\s*\(/.test(code))
                return null;
            return {
                id: "no_magic_number",
                name: "未设置 Magic Number",
    ...
    },
    {
        ...
        check(code, lines, ov) {
            ...
            if (!fixedLotRe.test(code))
                return null;
            if (/\bAccountInfoDouble\s*\(\s*ACCOUNT_BALANCE\b|\bNormalizeDouble\b.*\blot\b/i.test(code))
                return null;
    ...
    },
    {
        ...
        check(code, lines, ov) {
            ...
            if (!hasTrade)
                return null;
            if (/\bResultRetcode\s*\(\)|\bResultRetcodeDescription\s*\(/.test(code))
                return null;
    ...
    // ── LOW ───────────────────────────────────────────────────────────
    {
        ...
        check(code, lines, _ov) {
            if (!/\bCopyBuffer\s*\(/.test(code))
                return null;
            if (/\bArraySetAsSeries\s*\(/.test(code))
                return null;
    ...
    },
    {
        ...
        check(_code, _lines, ov) {
            if (!ov.hasOnInit || ov.hasOnDeinit)
                return null;
            if (!ov.usesIndicators && !ov.usesCTrade)
                return null;
            return {
                id: "nodeinit_missing",
                name: "有 OnInit 但无 OnDeinit",
...
function appendIssue(lines, issue, idx) {
