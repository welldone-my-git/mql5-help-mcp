...
class JavaPlugin {
        ...
        return [
            {
                ...
                description: "☕ Java 异常分析：粘贴完整的异常堆栈（StackTrace），自动提取异常类型与根因行，搜索本地文档并匹配历史修复模式，输出结构化诊断报告。",
                inputSchema: {
                    type: "object",
                    properties: {
                        stacktrace: {
            ...
            },
            {
                ...
                description: "🔍 Java 代码静态审查：检测空 catch、过宽异常捕获、System.out、原始泛型、循环字符串拼接、可变静态字段等7类常见问题，输出带行号的审查报告。完全本地，零 API 成本。",
                inputSchema: {
                    type: "object",
                    properties: {
                        code: {
    ...
    // ── Helpers ──────────────────────────────────────────────────────────────────
    diagnoseException(stacktrace, ctx) {
