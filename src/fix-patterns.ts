...
export interface FixPattern {
  id?: number;
  pattern_description: string;  // 问题的文字描述（用于模糊搜索）
  original_snippet?: string;    // 有问题的代码示例
  fixed_snippet?: string;       // 修复后的代码示例
  fix_description: string;      // 修复说明
  library_key?: string;         // 关联的库（可选）
  tags?: string;                // JSON 数组，如 ["handle_leak", "CTrade"]
  usage_count: number;
  created_at: string;
  last_used?: string;
}

export interface FixSearchResult extends FixPattern {
  relevance: number;
}
...
export class FixPatternsDb {
  ...
  record(data: Omit<FixPattern, "id" | "usage_count" | "created_at">): FixPattern {
    const db = this.init();
    const now = new Date().toISOString();

    // 去重：pattern_description 完全相同则更新
    const existing = db.prepare(
      "SELECT * FROM fix_patterns WHERE LOWER(pattern_description) = LOWER(?)"
    ).get(data.pattern_description) as FixPattern | undefined;
    ...
    const result = db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?)
    `).run(
      data.pattern_description, data.original_snippet, data.fixed_snippet,
      data.fix_description, data.library_key, data.tags, now, now
    );
    return { id: result.lastInsertRowid as number, ...data, usage_count: 1, created_at: now, last_used: now };
  }

  /** 关键词模糊搜索（OR 逻辑，按命中数排序） */
  search(keywords: string): FixSearchResult[] {
    const db = this.init();
    const terms = keywords.toLowerCase().split(/\s+/).filter(t => t.length > 2);
    if (terms.length === 0) return [];

    const conds = terms.map(() =>
      `(LOWER(pattern_description) LIKE ? OR LOWER(fix_description) LIKE ? OR LOWER(tags) LIKE ?)`
    ).join(" OR ");
    ...
    `).all(...params) as FixPattern[];

    return rows.map(row => ({
      ...row,
      relevance: terms.filter(t =>
        (row.pattern_description + " " + row.fix_description + " " + (row.tags ?? ""))
          .toLowerCase().includes(t)
      ).length / terms.length,
    })).sort((a, b) => b.relevance - a.relevance);
  }

  list(limit = 20): FixPattern[] {
    return this.init()
      .prepare("SELECT * FROM fix_patterns ORDER BY usage_count DESC, created_at DESC LIMIT ?")
      .all(limit) as FixPattern[];
  }
