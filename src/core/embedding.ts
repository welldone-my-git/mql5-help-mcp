...
export async function ollamaHealthCheck(url: string): Promise<boolean> {
  try {
...
export class VectorStore {
  ...
  private cache: Map<string, { embedding: Float32Array; absPath: string }> | null = null;
  ...
  upsert(
    ...
    opts?: { preview?: string; model?: string }
  ): void {
  ...
  hasKey(docKey: string): boolean {
    return !!this.db.prepare("SELECT 1 FROM doc_embeddings WHERE doc_key = ?").get(docKey);
  }

  loadAll(): Map<string, { embedding: Float32Array; absPath: string }> {
    if (this.cache) return this.cache;
    const rows = this.db.prepare(
      "SELECT doc_key, abs_path, embedding FROM doc_embeddings"
    ).all() as Array<{ doc_key: string; abs_path: string; embedding: Buffer }>;
...
export function hybridMerge(
  ...
  const map = new Map<string, HybridHit>();
...
export function extractTextForEmbedding(content: string, absPath: string): string {
