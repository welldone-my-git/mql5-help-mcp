...
class CTrade:
    ...
    def __init__(self, magic_number: int, filling_type_symbol: str, deviation_points: int, mt5_instance: mt5=mt5):
        
        """"CTrade class provides an interface to perform trading operations in MetaTrader5.
            Similary to the CTrade class in MQL5, it allows opening and managing market positions and pending orders.
            
            for more information, see: https://www.mql5.com/en/docs/standardlibrary/tradeclasses/ctrade
        """
    ...
    def order_open(self, symbol: str, volume: float, order_type: int, price: float, sl: float = 0.0, tp: float = 0.0, type_time: int = mt5.ORDER_TIME_GTC, expiration: datetime = None, comment: str = "") -> bool:
        ...
        request = {
            ...
            "deviation": self.deviation_points,
            "magic": self.magic_number,
            "comment": comment[:31],  # self.mt5_instance comment max length is 31 chars
            "type_time": type_time,
            "type_filling": self.filling_type,
        }
    ...
    def position_close(self, ticket: int, deviation: float=float("nan")) -> bool:
        ...
        if not self.mt5_instance.positions_get(ticket=ticket):
            print(f"Position with ticket {ticket} not found.")
            return False

        position = self.mt5_instance.positions_get(ticket=ticket)[0]
        symbol = position.symbol
        volume = position.volume
        position_type = position.type  # 0=BUY, 1=SELL

        # Get close price (BID for buy, ASK for sell)
        price = self.mt5_instance.symbol_info_tick(symbol).bid if position_type == self.mt5_instance.POSITION_TYPE_BUY else self.mt5_instance.symbol_info_tick(symbol).ask
    ...
    def order_delete(self, ticket: int) -> bool:
        ...
        # Check result
        if result is None:
            print(f"order_delete() failed, error: {self.mt5_instance.last_error()}")
            return False
    ...
    def position_modify(self, ticket: int, sl: float, tp: float) -> bool:
        ...
        if result is None:
            print(f"order_modify() failed, error: {self.mt5_instance.last_error()}")
            return False
    ...
    def order_modify(self, ticket: int, price: float, sl: float, tp: float, type_time: int = mt5.ORDER_TIME_GTC, expiration: datetime = None, stoplimit: float = 0.0) -> bool:
        ...
        order = self.mt5_instance.orders_get(ticket=ticket)
        if not order:
