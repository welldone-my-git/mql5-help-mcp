...
//|                                                                  |
//|  ── RNA classes (recurrence network analysis) ─────────────────  |
//|    CRNAMetrics  — graph metrics from recurrence matrix           |
//|    CRNAWindow   — rolling/windowed RNA analysis (single series)  |
//|    CRNA         — high-level all-in-one RNA facade               |
//|                                                                  |
//|  ── JRNA classes (joint recurrence network analysis) ──────────  |
//|    CJRNAWindow  — rolling/windowed JRNA (two series)             |
//|    CJRNA        — high-level all-in-one JRNA facade              |
//|                                                                  |
//|  ── Structs ──────────────────────────────────────────────────── |
...
//|    SRNAResult        — holds all RNA network metric values       |
//|    SRNAWindowResult  — per-window RNA metric values              |
//|                                                                  |
//|  ── Enums ────────────────────────────────────────────────────── |
...
// --- Recurrence Network Analysis modules ---
#include "RNAMetrics.mqh"
#include "RNAWindow.mqh"
#include "JRNAWindow.mqh"
...
//+------------------------------------------------------------------+
//| CRNA — high-level facade for Recurrence Network Analysis          |
//|                                                                  |
//|  Reinterprets the recurrence matrix as a complex network and     |
//|  computes graph-theoretic measures: clustering, transitivity,    |
//|  path lengths, betweenness centrality, assortativity.            |
//|                                                                  |
//|  Quick usage:                                                    |
//|    CRNA rna;                                                     |
//|    rna.SetEpsilon(0.05);                                         |
//|    rna.SetEmbedding(2, 1);                                       |
//|    if(rna.Compute(close, len))                                   |
//|      rna.PrintSummary();                                         |
//|    double acc = rna.AvgClustering();                              |
//|    double apl = rna.AvgPathLength();                              |
//+------------------------------------------------------------------+
class CRNA
  {
private:
   CRQAMatrix        m_matrix;
   CRNAMetrics       m_metrics;
   SRNAResult        m_result;
   bool              m_computed;
...
public:
   ...
   void              GetResult(SRNAResult &out) const { out = m_result; }

   double            AvgDegree()      const { return m_result.AvgDegree; }
   double            MaxDegree()      const { return m_result.MaxDegree; }
   double            DegreeStd()      const { return m_result.DegreeStd; }
   double            AvgClustering()  const { return m_result.AvgClustering; }
   double            Transitivity()   const { return m_result.Transitivity; }
   double            AvgPathLength()  const { return m_result.AvgPathLength; }
   double            Diameter()       const { return m_result.Diameter; }
   double            AvgBetweenness() const { return m_result.AvgBetweenness; }
   double            MaxBetweenness() const { return m_result.MaxBetweenness; }
   double            AvgCloseness()   const { return m_result.AvgCloseness; }
   double            Assortativity()  const { return m_result.Assortativity; }
   double            Density()        const { return m_result.Density; }
...
//+------------------------------------------------------------------+
//| Build recurrence matrix and compute all network metrics           |
//+------------------------------------------------------------------+
bool CRNA::Compute(const double &series[], int N)
  {
   m_computed = false;
   m_result.Reset();
   ...
   if(N < 4)
     {
      Print("CRNA::Compute — series too short (min 4 bars)");
      return false;
     }
...
//+------------------------------------------------------------------+
//| Print all RNA metrics to the Experts log                          |
//+------------------------------------------------------------------+
void CRNA::PrintSummary() const
  {
   if(!m_computed)
     {
      Print("CRNA: Not computed yet — call Compute() first");
      return;
     }
   PrintFormat("===== Recurrence Network Analysis =====");
   PrintFormat("Epsilon        : %.6f  (embDim=%d, delay=%d)",
               m_epsilon, m_embDim, m_delay);
   PrintFormat("Avg Degree     : %.4f",  m_result.AvgDegree);
   PrintFormat("Max Degree     : %.0f",  m_result.MaxDegree);
   PrintFormat("Degree Std     : %.4f",  m_result.DegreeStd);
   PrintFormat("Avg Clustering : %.4f",  m_result.AvgClustering);
   PrintFormat("Transitivity   : %.4f",  m_result.Transitivity);
   PrintFormat("Avg Path Length: %.4f",  m_result.AvgPathLength);
   PrintFormat("Diameter       : %.0f",  m_result.Diameter);
   PrintFormat("Avg Betweenness: %.4f",  m_result.AvgBetweenness);
   PrintFormat("Max Betweenness: %.4f",  m_result.MaxBetweenness);
   PrintFormat("Avg Closeness  : %.4f",  m_result.AvgCloseness);
   PrintFormat("Assortativity  : %.6f",  m_result.Assortativity);
   PrintFormat("Density        : %.4f",  m_result.Density);
   PrintFormat("=======================================");
  }

//+------------------------------------------------------------------+
//| CJRNA — high-level facade for Joint Recurrence Network Analysis   |
//|                                                                  |
//|  Builds a joint recurrence matrix from two time series, then     |
//|  treats it as a network and computes graph-theoretic measures.    |
//|  Reveals the shared dynamical structure between two markets.     |
//|                                                                  |
//|  Quick usage:                                                    |
//|    CJRNA jrna;                                                   |
//|    jrna.SetEpsilon(0.05);                                        |
//|    jrna.SetEmbedding(2, 1);                                      |
//|    if(jrna.Compute(closeX, closeY, len))                         |
//|      jrna.PrintSummary();                                        |
//|    double acc = jrna.AvgClustering();                             |
//+------------------------------------------------------------------+
class CJRNA
  {
private:
...
public:
   ...
   bool              Compute(const double &seriesX[],
...
//+------------------------------------------------------------------+
//| Build joint recurrence matrix, extract adjacency, compute metrics |
//+------------------------------------------------------------------+
bool CJRNA::Compute(const double &seriesX[],
                     const double &seriesY[],
                     int seriesLen)
  {
   m_computed = false;
   ...
   if(seriesLen < 4)
     {
      Print("CJRNA::Compute — series too short (min 4 bars)");
      return false;
     }
   ...
   for(int i = 0; i < N; i++)
      for(int j = 0; j < N; j++)
         adj[i * N + j] = (char)((i != j && m_matrix.Get(i, j)) ? 1 : 0);

   if(!m_metrics.ComputeFromAdj(adj, N, m_result))
      return false;
...
//+------------------------------------------------------------------+
//| Print all JRNA metrics to the Experts log                         |
//+------------------------------------------------------------------+
void CJRNA::PrintSummary() const
  {
   if(!m_computed)
     {
      Print("CJRNA: Not computed yet — call Compute() first");
      return;
     }
   PrintFormat("===== Joint Recurrence Network Analysis =====");
   PrintFormat("EpsilonX       : %.6f  EpsilonY: %.6f  (embDim=%d, delay=%d)",
               m_epsilonX, m_epsilonY, m_embDim, m_delay);
   PrintFormat("Avg Degree     : %.4f",  m_result.AvgDegree);
   PrintFormat("Max Degree     : %.0f",  m_result.MaxDegree);
